-- CoachWave Platform Schema Migration - Fixed Types
-- This migration creates the coaching platform schema matching existing types
...
-- Add reference columns that match existing types
ALTER TABLE public.sessions 
ADD COLUMN IF NOT EXISTS coach_profile_id bigint REFERENCES public.coach_profiles(id) ON DELETE CASCADE,
ADD COLUMN IF NOT EXISTS client_profile_id uuid REFERENCES public.profiles(user_id) ON DELETE CASCADE;

-- Create reviews table with matching types
CREATE TABLE IF NOT EXISTS public.reviews (
  id bigserial PRIMARY KEY,
  session_id bigint REFERENCES public.sessions(id) ON DELETE CASCADE UNIQUE NOT NULL,
  client_id uuid REFERENCES public.profiles(user_id) ON DELETE CASCADE NOT NULL,
  coach_id bigint REFERENCES public.coach_profiles(id) ON DELETE CASCADE NOT NULL,
